...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        """
        currentNode, visitedCorners = state[0], list(state[1])
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in visitedCorners:
                    successors.append((((nextx, nexty), tuple(visitedCorners + [(nextx, nexty)])), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    distances = [mazeDistance(corner, coordinates, problem.startingGameState) for corner in unvisited_corners]
